...
def energy_imshow(
    ...
    elif data.shape == (10, 7, 9):
        pass

    else:
        raise ValueError(
            f"Ожидалась форма (10,7,9) "
            f"или (7,9,10), получена {data.shape}"
        )
...
def get_test_data(global_config):
    ...
    data_train = np.load(
        os.path.join(
            ...
            'fhcal_data_side_modules81619.npz'
        )
    )
...
def plot_fhcal_simple(energy_data, event_idx=None):
    ...
    if data.shape == (7, 9, 10):
        ...
        raise ValueError(
            f"Ожидалась форма (10,7,9) или (7,9,10), "
            f"получена {data.shape}"
        )
